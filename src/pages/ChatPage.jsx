...
export default function ChatApp() {
    ...
    return (
        <section className="w-full flex pt-20 md:pt-30 h-screen px-15 mb-10">
            <div className="w-1/3">
            ...
            <div className="w-2/3 relative">
                <div className="flex flex-col justify-between bg-[#ffffff] h-full overflow-hidden">
                    ...
                    <div className="flex flex-col h-full overflow-y-auto p-5 space-y-2 w-full" ref={chatRef}>
                        {messages.map((msg, index) => (
                            <div
                                key={index}
                                className={`py-2 flex flex-col justify-between px-3 rounded-lg max-w-xs text-sm ${msg.sender === "user"
                    ...
                    <div className="flex items-center gap-2 mt-2 text-black/70 bg-[#fff] border-t border-t-black/30 w-full p-5">
                        <button>
                            <Camera />
                        </button>
                        <button>
                        ...
                        <input
                            type="text"
                            className="flex-1 p-2 bg-gray-300 rounded-lg"
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            onKeyDown={(e) => e.key === "Enter" && sendMessage()}
                            placeholder="Type message..."
                        />
                        <button onClick={sendMessage} className="p-2 px-4 cursor-pointer bg-green text-white rounded-lg">
                            <SendHorizonal />
                        </button>
                    </div>
                </div>
